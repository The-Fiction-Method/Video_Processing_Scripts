...
CALL :CHANopus %CHAN%
set opusMF=-mapping_family -1
::	the ffmpeg default
CALL :RATEopusChan rateA 0 0
...
::	<rate> <crf> <preset>
CALL :X265process "%~1" 3000k 18 slow
CALL :X265process "%~1" 3000k 21 slow
CALL :X265process "%~1" 3000k 24 slow
REM CALL :X265pass "%~1" %rate% 18 slow

::	for UHD-HDR conversion
REM set FILT=%FILT%, libplacebo=w=-1:h='min(ih,1080)':color_primaries=bt709:color_trc=bt709:colorspace=bt709:tonemapping=bt.2446a:downscaler=lanczos:format=yuv420p10le
REM CALL :X265process "%~1" 50000k 12 slow
::	added `-init_hw_device vulkan` as it is necessary for libplacebo
::	bt.2446a suggested as better than hable
::	old below
REM set FILT=%FILT%, zscale=t=linear:npl=100,format=gbrpf32le,zscale=p=bt709,tonemap=tonemap=hable:desat=0,zscale=t=bt709:m=bt709,format=yuv420p
REM ::set FILT=%FILT%,eq=gamma=1.02:contrast=0.955:eval=frame
REM CALL :X265scale "%~1" 1080 50000k 12 slow

REM pause
::	<scale> <rate> <crf> <preset>
REM CALL :X265scale "%~1" 720 3000k 18 slow
REM CALL :X265scale "%~1" 720 3000k 21 slow
REM CALL :X265scale "%~1" 720 3000k 24 slow
...
:CHANopus <channels>
::	for setting the mapping_family option for opus encoder
::	supposedly this will help with surround-sound encoding, but it seems it should be automatic
set chans=%~1

set opusMF=-mapping_family 0
if %chans% GTR 2 set opusMF=-mapping_family 1
exit /B 0
...
-metadata comment="%preset% %crf% %rateO%%X265config%%CONFIG%%scaleCON%" ^
-an "%folder%\%~n1 %preset% %crf% %rateO%%X265config%%CONFIG%%scaleCON%.mkv" -n
...
ffmpeg -hide_banner -i "%~1" -filter_complex "%FILT%" -c:s copy ^
-c:v libx265 -pix_fmt yuv420p10le -crf %crf% %rate% -preset %preset% %TUNE% ^
-x265-params pass=2:stats="%~n1.log"%X265param% ^
-an "%folder%\%~n1 %preset% %crf% %rateO%%X265config%%CONFIG%.mkv"
