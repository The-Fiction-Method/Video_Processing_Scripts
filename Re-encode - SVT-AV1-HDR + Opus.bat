...
::	?scale?	<crf>	<preset>
CALL :AV1svt "%~1" 34 4
REM CALL :AV1svtVBRscale "%~1" 720 34 4

::			34		4			3000
::	?scale?	<crf>	<preset>	<rate>
REM CALL :AV1svtMBR "%~1" 34 4 %rate%
REM CALL :AV1svtMBRscale "%~1" 480 34 4 %rate%

::	?scale? <rate> <crf> <preset>
REM CALL :AV1svtpass "%~1" %rate%
REM CALL :AV1svtpassscale "%~1" 480 %rate%
...
-map 0:a:0? -ac %CHAN% -c:a libopus %opusMF% -b:a %rateA% -vbr 1 ^
-map 0:s? -c:s copy ^
...
set rate=%~2
if NOT "%rate%"=="" set rate=--mbr %rate%
set crf=%~3
if "%crf%"=="" set crf=20
set preset=%~4
if "%preset%"=="" set preset=4

ffmpeg -hide_banner -i "%~1" -vf "%FILT%" -pix_fmt yuv420p10le -f yuv4mpegpipe -strict -1 - | SvtAv1EncApp --input - --preset %preset% --crf %crf% --tune %TUNE% --noise-norm-strength %NOISENORM% --psy-rd %PSYRD% --variance-octile 3 %rate% --fast-decode 1 --film-grain-denoise 0 %GRAIN%  --pass 1

ffmpeg -hide_banner -i "%~1" -vf "%FILT%" -pix_fmt yuv420p10le -f yuv4mpegpipe -strict -1 - | SvtAv1EncApp --input - --preset %preset% --crf %crf% --tune %TUNE% --noise-norm-strength %NOISENORM% --psy-rd %PSYRD% --variance-octile 3 %rate% --fast-decode 1 --film-grain-denoise 0 %GRAIN%  --pass 2 --output - | ^
ffmpeg -hide_banner -i - -i "%~1" -map 0:v -c:v copy -map 1:s? -c:s copy -map 1:a -ac %CHAN% -c:a libopus %rateA% -vbr 1 -map_metadata 1 "%folder%\%~n1.mkv"
